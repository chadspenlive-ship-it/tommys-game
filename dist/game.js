...
const MISSION_MENU_TAP_SLOP = 8;
const COLOR_DELIVERY_SEQUENCE = ["red", "blue", "yellow", "green", "purple", "orange"];
const NUMBER_PARKING_SEQUENCE = [7, 2, 5, 1, 8, 3];
const PAN_MODES = [
...
const MISSIONS = [
  {
    ...
    color: "#58d6ff",
    start: { x: 0, y: 560 },
    bounds: { minX: -980, maxX: 980, minY: -20500, maxY: 900 },
    speedScale: 0.54,
    path: [
      { x: 0, y: 560 },
      { x: -240, y: 180 },
  ...
  },
  {
    ...
    start: { x: 0, y: -150 },
    targetNumber: 7,
    parkingSequence: NUMBER_PARKING_SEQUENCE,
  ...
  },
  {
    ...
    start: { x: 0, y: 660 },
    targetColor: "red",
    deliverySequence: COLOR_DELIVERY_SEQUENCE,
  ...
  },
  {
    ...
    color: "#36d16f",
    start: { x: 0, y: 640 },
    bounds: { minX: -620, maxX: 620, minY: -5420, maxY: 980 },
    speedScale: 0.44,
    path: [
      { x: 0, y: 640 },
      { x: 0, y: 80 },
...
function spawnMissionCop() {
  const playerWorld = getPlayerWorld();
  cop.pending = false;
  cop.active = true;
  cop.exploded = false;
  cop.outrunTimer = 0;
  cop.x = clamp(playerWorld.x + (playerWorld.x < 0 ? 360 : -360), -520, 520);
  cop.y = playerWorld.y + 760;
  cop.vx = 0;
  cop.vy = -720;
  cop.speed = 720;
  cop.angle = 0;
...
function updateMissionCop(dt) {
  if (!cop.active || gameState.phase !== "mission") {
    return;
  }

  const playerWorld = getPlayerWorld();
  const seekX = playerWorld.x - cop.x;
  const seekY = playerWorld.y - cop.y;
  const distance = Math.hypot(seekX, seekY) || 1;
  const desiredAngle = Math.atan2(seekX, -seekY);
  cop.angle = angleLerp(cop.angle, desiredAngle, 1 - Math.exp(-dt * 4.2));
  const chaseSpeed = clamp(680 + Math.abs(car.speed) * 0.28, 720, 1120);
  cop.vx = lerp(cop.vx, (seekX / distance) * chaseSpeed, 1 - Math.exp(-dt * 3.1));
  cop.vy = lerp(cop.vy, (seekY / distance) * chaseSpeed, 1 - Math.exp(-dt * 3.1));
  cop.x += cop.vx * dt;
  cop.y += cop.vy * dt;
  cop.speed = Math.hypot(cop.vx, cop.vy);
  cop.horn = 0.75 + Math.sin(performance.now() * 0.016) * 0.25;

  const hitDistance = (cop.width + car.width) * 0.45 + (cop.length + car.length) * 0.12;
  if (distance < hitDistance) {
    failMission("Busted!", "The police car bumped you after running the red light.");
  }
}
...
function getMissionParkingSequence(mission = getActiveMission()) {
  return mission.parkingSequence || [mission.targetNumber || 1];
}

function getMissionDeliverySequence(mission = getActiveMission()) {
  return mission.deliverySequence || [mission.targetColor || "red"];
}

function getCurrentParkingNumber(mission = getActiveMission()) {
  const sequence = getMissionParkingSequence(mission);
  return sequence[Math.min(gameState.missionProgress, sequence.length - 1)] || mission.targetNumber || 1;
}

function getCurrentDeliveryColor(mission = getActiveMission()) {
  const sequence = getMissionDeliverySequence(mission);
  return sequence[Math.min(gameState.missionProgress, sequence.length - 1)] || mission.targetColor || "red";
}
...
function getMissionPrompt(mission = getActiveMission()) {
  const prompts = {
    ...
    numberParking: `Park at ${getCurrentParkingNumber(mission)}.`,
    colorDelivery: `Pick up ${getCurrentDeliveryColor(mission)}, then drive to ${getCurrentDeliveryColor(mission)}.`,
...
function getColorGaragePositions() {
  return [
    ...
    { key: "green", label: "GREEN", color: "#36d16f", x: -520, y: -520 },
    { key: "purple", label: "PURPLE", color: "#b56cff", x: 520, y: 80 },
    { key: "orange", label: "ORANGE", color: "#ff9a30", x: -520, y: 80 },
  ];
}
...
function startMission(id) {
  ...
  gameState.missionTargetNumber = getCurrentParkingNumber(mission);
  gameState.missionTargetColor = getCurrentDeliveryColor(mission);
  gameState.missionCarryItem = false;
  gameState.missionSuccessText = "";
...
function failMission(message = "Mission Failed", messageSub = "You got busted before finishing the mission.") {
  activeTouches.clear();
  car.vx *= 0.2;
  car.vy *= 0.2;
  cop.active = false;
  cop.pending = false;
  cop.pendingTimer = 0;
  setPhase("missionFail", message, messageSub);
}
...
function updateNumberParkingMission(mission) {
  ...
  const sequence = getMissionParkingSequence(mission);
  const targetNumber = getCurrentParkingNumber(mission);
  const target = spaces.find((space) => space.number === targetNumber) || spaces[0];
  gameState.missionTargetNumber = targetNumber;
  if (Math.hypot(playerWorld.x - target.x, playerWorld.y - target.y) < 92 && Math.abs(car.speed) < 24 && activeTouches.size === 0) {
    gameState.missionSuccessText = `Good job parking at ${targetNumber}!`;
    gameState.missionNoticeTimer = 1.35;
    playConeHit(0.7);
    gameState.missionProgress += 1;
    if (gameState.missionProgress >= sequence.length) {
      completeMission("Great job parking in every number!");
      return;
    }
    gameState.missionTargetNumber = getCurrentParkingNumber(mission);
  }
...
function updateColorDeliveryMission(mission) {
  ...
  const sequence = getMissionDeliverySequence(mission);
  const targetColor = getCurrentDeliveryColor(mission);
  const garage = getColorGaragePositions().find((item) => item.key === targetColor);
  gameState.missionTargetColor = targetColor;
  if (!gameState.missionCarryItem && Math.hypot(playerWorld.x - pickup.x, playerWorld.y - pickup.y) < 120) {
    ...
    gameState.missionSuccessText = `${targetColor.toUpperCase()} package picked up!`;
    playConeHit(0.7);
  } else if (gameState.missionCarryItem && garage && Math.hypot(playerWorld.x - garage.x, playerWorld.y - garage.y) < 135) {
    gameState.missionCarryItem = false;
    gameState.missionSuccessText = `Good job delivering ${targetColor}!`;
    gameState.missionNoticeTimer = 1.45;
    playConeHit(0.7);
    gameState.missionProgress += 1;
    if (gameState.missionProgress >= sequence.length) {
      completeMission("Every color reached its matching house!");
      return;
    }
    gameState.missionTargetColor = getCurrentDeliveryColor(mission);
  }
...
function updateStopGoMission(mission, dt) {
  if (gameState.missionStep === "copChase") {
    return;
  }
  ...
  if (light && Math.abs(playerWorld.y - light.y) < 120 && Math.abs(playerWorld.x - light.x) < 190) {
    if (gameState.missionLightGreen) {
      ...
      gameState.missionStep = "copChase";
      gameState.missionMistakeTimer = 2.4;
      spawnMissionCop();
      playConeHit(0.35);
    }
  }
...
function drawCurrentMissionTarget(mission) {
  ...
  if (mission.path) {
    ...
    target = gameState.missionCarryItem
      ? getColorGaragePositions().find((item) => item.key === gameState.missionTargetColor)
...
function drawColorDeliveryScene(mission) {
  const pickup = getColorPickupPosition(mission);
  const currentColor = getColorGaragePositions().find((item) => item.key === gameState.missionTargetColor) || getColorGaragePositions()[0];
  ...
  for (const garage of getColorGaragePositions()) {
    const active = garage.key === gameState.missionTargetColor;
    ctx.fillStyle = garage.color;
    ctx.beginPath();
    ctx.moveTo(garage.x - 180, garage.y - 118);
    ctx.lineTo(garage.x, garage.y - 228);
    ctx.lineTo(garage.x + 180, garage.y - 118);
    ctx.closePath();
    ctx.fill();
...
function drawMissionHud() {
  ...
  if (gameState.missionStep === "findStall" || mission.type === "numberParking" || mission.type === "stopGo" || gameState.missionNoticeTimer > 0) {
    const flash = 0.42 + Math.sin(performance.now() * 0.012) * 0.28;
    ctx.textAlign = "center";
...
function getMissionHudMessage(mission) {
  ...
  if (mission.type === "numberParking") {
    const sequence = getMissionParkingSequence(mission);
    return `Park at ${gameState.missionTargetNumber}, lift finger (${gameState.missionProgress + 1}/${sequence.length})`;
  }
  if (mission.type === "colorDelivery") {
    const sequence = getMissionDeliverySequence(mission);
    const color = gameState.missionTargetColor || getCurrentDeliveryColor(mission);
    return gameState.missionCarryItem
      ? `Take ${color} to ${color} house (${gameState.missionProgress + 1}/${sequence.length})`
      : `Pick up the ${color} package (${gameState.missionProgress + 1}/${sequence.length})`;
  }
  if (mission.type === "stopGo") {
    if (gameState.missionStep === "copChase") {
      return "Police chase! Do not get bumped.";
    }
...
function getMissionFlashMessage(mission) {
  if (gameState.missionNoticeTimer > 0 && gameState.missionSuccessText) {
    return gameState.missionSuccessText.toUpperCase();
  }
  if (mission.type === "grocery") {
    return `STALL ${gameState.missionStall}`;
  }
  if (mission.type === "numberParking") {
    return `PARK AT ${gameState.missionTargetNumber}`;
  }
  if (mission.type === "stopGo") {
    if (gameState.missionStep === "copChase") {
      return "BUSTED?";
...
function handlePointerMove(event) {
  if (missionMenuDrag && missionMenuDrag.id === event.pointerId) {
    ...
    const totalDistance = Math.hypot(event.clientX - missionMenuDrag.startX, event.clientY - missionMenuDrag.startY);
    missionMenuDrag.moved = missionMenuDrag.moved || totalDistance > MISSION_MENU_TAP_SLOP;
...
function handlePointerUp(event) {
  ...
  if (missionMenuDrag && missionMenuDrag.id === event.pointerId) {
    const drag = missionMenuDrag;
    missionMenuDrag = null;
    const button = getOverlayButtonAt(event.clientX, event.clientY);
    if (event.type === "pointerup" && !drag.moved && drag.action && button?.action === drag.action) {
      handleOverlayAction(drag.action);
    }
  }
}
